...
前端 (React + Zustand)
   ...
   Agent             │                 (+ teaching     Agent
    ...
    (in-process)   (PostgreSQL)      (PostgreSQL)                │
...
- PostgreSQL 原生 row-level locking + asyncpg pool：並發 acquire/release/cleanup_stale 無 deadlock（不再依賴 SQLite WAL）
...
**跨重啟的持久狀態**：全部存於 PostgreSQL（asyncpg pool，連線由 `DATABASE_URL` 設定）
...
uvicorn run:app --port 8000
    ...
    │
    └── lifespan(app)
            ├── init_db(os.getenv("DATABASE_URL"))  # 建 asyncpg pool（runtime 讀 DATABASE_URL）、冪等套用 schema.sql baseline
...
### 資料庫 Schema（單一 baseline）

資料庫已於 2026-06 從 SQLite/aiosqlite 遷移到 **PostgreSQL/asyncpg**。schema **不再使用增量 migration runner**，改為單一 baseline `backend/db/schema.sql`（13 張表），由 `init_db` 啟動時以 `CREATE TABLE IF NOT EXISTS` 冪等套用；沒有 `schema_migrations` 追蹤表、沒有逐步 `ALTER TABLE`。連線為 asyncpg pool（`get_db()` 回傳 pool；以 `DATABASE_URL` / `DB_POOL_MIN_SIZE` / `DB_POOL_MAX_SIZE` 設定）。

測試以 **testcontainers** 起拋棄式 PostgreSQL（故跑測試需 Docker daemon，依賴在 `backend/requirements-dev.txt`）；`init_db(dsn, *, reset=True)` 僅在 `WL_TEST_ENV=1` 下允許 drop/recreate public schema。

> 下方歷史 migration 表與本文件 schema 段落中散落的「（Migration NNN）」標註，**僅供考古**（SQLite 時代的逐步演進），非現行執行步驟；實際 schema 一律以 `schema.sql` 為準。

<details>
<summary>歷史 migration 沿革（SQLite 時代，已停用）</summary>
...
（以上為 SQLite 時代的逐步演進；PostgreSQL 遷移後這些步驟已全部收斂進 `schema.sql` baseline，不再逐步執行。）
...
> **PostgreSQL 型別對照**：以下欄位定義承襲 SQLite 時代寫法（`TEXT` / `INTEGER` / `TIMESTAMP`）以保留語義，實際 `schema.sql` 中時間欄位多為 `TIMESTAMPTZ DEFAULT now()`、自增主鍵為 `BIGINT GENERATED BY DEFAULT AS IDENTITY`、JSON 一律以 `TEXT` 儲存（不轉 JSONB）。各處「（Migration NNN）」為歷史沿革標註，現行 schema 以 `backend/db/schema.sql` 為準。
...
```
註冊白名單：`/auth/register` 只允許 email 命中此表者成功註冊，角色（`admin` / `user`）一併寫入 user。`user` 角色不可新增教材（upload / 建課綱），但可閱讀既有教材、答題、提問；`admin` 可用全部功能。帳號與權限直接在 DB 維護。
...
| `upsert_stage_progress(...)` | upsert stage_progress（`INSERT ... ON CONFLICT DO UPDATE`） |
...
| `DATABASE_URL` | `postgresql://wl:wl@localhost:5432/wl` | PostgreSQL 連線（asyncpg）。`main.py` lifespan 與 arq worker startup 皆 **runtime** 讀 `os.getenv` |
| `DB_POOL_MIN_SIZE` | `1` | asyncpg pool 最小連線數 |
| `DB_POOL_MAX_SIZE` | `10` | asyncpg pool 最大連線數 |
...
- **Docker**（`docker-compose.yml`）：
  - `wl-postgres`：host `:5432` → container `:5432`，資料落 named volume `pg_data`，`pg_isready` healthcheck
  - `wl-redis`：host `:6380` → container `:6379`
  - `wl-curriculum-worker` / `wl-api`：`depends_on postgres(healthy)`、`./backend:/app/backend`、`hf_cache`；以 `DATABASE_URL=postgresql://wl:wl@postgres:5432/wl` 連 DB
  - entrypoint（`backend/docker/worker-entrypoint.sh`）：`arq_settings.startup` runtime 讀 `DATABASE_URL`（未經 compose 直跑時 fallback 一個預設值），不再有 SQLite 種子複製 / `wal_checkpoint`
- **uvicorn 與 worker 共用同一 PostgreSQL**：PG 原生處理多 process 併發寫，無 SQLite 時代「勿同時寫」限制；Arq 模式下 uvicorn 仍只 enqueue（生成由 worker 跑）
