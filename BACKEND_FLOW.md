...
Migration 002–005、007–008、010 均用 `try/except` 包裹，已存在欄位時靜默跳過（冪等）。006、009 使用 `CREATE TABLE IF NOT EXISTS`。注意：程式碼中 006（`decision_records`）的建立語句實際位於 010 之後，但邏輯上屬於 Migration 006。
...
class WorkingMemory:
    ...
    remediate_count: int = 0             # 當前 stage 已補強次數；>= 2 時 ProgressManager 強制 advance
```
...
- 刪除 session 及其所有 qa_records、stage_progress、source_chunks、decision_records
...
|----------|----------------------|-------------|
| ClaudeProvider | 獨立 `system` 參數 | `claude-sonnet-4-6` |
| OpenAIProvider | messages[0] 插入 system role | `gpt-5.4-mini` |
| GeminiProvider | `config.system_instruction`（新 `google.genai` SDK） | `gemini-3-flash-preview` |
| MonicaProvider | OpenAI 相容格式（透過 `MONICA_BASE_URL`/`MONICA_API_KEY`） | `claude-4.6-sonnet` |
...
> Phase 1 後，Files API 降級為 fallback，主路徑改為後端 text_extractor + chunker。Monica 使用 OpenAI 相容 API，需在 `.env` 設定 `MONICA_BASE_URL` 與 `MONICA_API_KEY`。
...
| `DEFAULT_PROVIDER` | `claude` | 預設 LLM Provider（claude｜openai｜gemini｜monica） |
