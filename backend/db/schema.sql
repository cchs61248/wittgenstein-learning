...
-- 註：qa_records / decision_records / tutor_records / llm_result_cache 刻意不設
-- session_id FK（沿用原 SQLite 設計，視為 append-only 稽核資料）。session 刪除時
-- 由 session_memory 的刪除流程手動清理，而非靠 ON DELETE CASCADE。
